...
// FAQS
const FAQS = [
  { q: "┬┐C├│mo veo mis lecturas y gr├íficas?", a: "Entra a tu Dashboard de usuario." },
  { q: "┬┐Qu├® significa 'Bater├¡a baja'?", a: "Que tu bater├¡a est├í bajo el 20%." },
  { q: "┬┐Puedo exportar mis datos?", a: "S├¡, el administrador puede generar reportes CSV y PDF." },
  { q: "┬┐No puedo iniciar sesi├│n?", a: "Usa 'Olvidaste tu contrase├▒a' en la pantalla de acceso." },
];

// Formato fecha
const fmtFecha = (iso) => (iso ? new Date(iso).toLocaleString() : "ÔÇö");
...
export default function Contactos() {
  ...
  const onProbarConexion = async () => {
    ...
    try {
      ...
      setEstadoAPI("ok");
      setEstadoMsg(`Conectado. Usuario: ${r?.data?.usuario || "ÔÇö"}`);
      pushHist("probar", "Conexi├│n OK");
    } catch {
      setEstadoAPI("fail");
      setEstadoMsg("No se pudo conectar al backend.");
      pushHist("probar", "Conexi├│n FALL├ô");
    } finally {
      setProbando(false);
    }
  };
  ...
  return (
    <div style={{ backgroundColor: "#f8f9fa", minHeight: "100vh", padding: "40px 20px" }}>
      <div className="container" style={{ maxWidth: 1024 }}>
        <h2 className="text-center mb-4 text-primary">­ƒøá´©Å Centro de Ayuda ÔÇö Sistema E├│lico</h2>
                    ...
                    <tbody>
                      {notas.map((n, i) => (
                        <tr key={n.id}>
                          ...
                          <td className="text-muted">{n.txt.length > 80 ? n.txt.slice(0, 80) + "ÔÇª" : n.txt}</td>
                          <td>
